...
define([
    ...
    return {
        ...
        resolveShippingRates: function (ratesData) {
            ...
            if (selectedShippingRate === "in_store_pickup_in_store_pickup") {
                shippingMethod = ratesData.filter(item => item.method_code === 'in_store_pickup');
                selectShippingMethodAction(shippingMethod[0]);
                return ;
            }
            ...
            if (ratesData.length >= 0 && !selectedShippingRate) {
                shippingMethod = ratesData.filter(item => item.method_code !== 'in_store_pickup');
                selectShippingMethodAction(shippingMethod[0]);
                return ;
            }

            if (selectedShippingRate && quote.shippingMethod() && quote.shippingMethod()['method_code'] === "in_store_pickup") {
                // skip default action if selectedShippingRate is in store pickup
            }
