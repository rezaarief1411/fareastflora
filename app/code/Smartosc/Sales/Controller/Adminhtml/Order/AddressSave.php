...
class AddressSave extends Order implements HttpPostActionInterface
{
    ...
    public function execute()
    {
        ...
        try {
            $addressBuilding = $order->setData($address->getAddressType() . '_building', $data['building']);
            $addressFloor = $order->setData($address->getAddressType() . '_floor', $data['floor']);
            $order->save();
        } catch (LocalizedException $e) {
            $this->messageManager->addErrorMessage($e->getMessage());
        } catch (\Exception $e) {
            $this->messageManager->addExceptionMessage($e, __('We can\'t update the order address building & floor right now.'));
        }
