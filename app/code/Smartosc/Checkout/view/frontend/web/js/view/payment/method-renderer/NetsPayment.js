define(
    ...
    function (
        ...
        if (customer.isLoggedIn()) {
            $.ajax({
                type: "POST",
                url: url.build('nets/token/index'),
        ...
        return Component.extend({
            defaults: {
                template: 'Nets_NetsPayment/payment/NetsPayment'
            },
            ...
            removeStoredCard: function () {
                ...
                $.ajax({
                    type: "POST",
                    url: url.build('nets/token/remove'),
                    data: { token: tokenId },
                    async: false,
                    success: function (response) {
                        if (response === "0") {
            ...
            afterPlaceOrder: function () {
                window.location.replace(url.build('nets/payment/request'));
            }
        });
    }
);
